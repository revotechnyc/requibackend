...
### HTTPS on port 8000 (requi.io)

If `/etc/letsencrypt/live/requi.io/` exists, `python scripts/run_uvicorn.py` serves **HTTPS on port 8000** (same cert paths as Node). Frontend build: `VITE_API_URL=https://requi.io:8000`. Docker Compose mounts `/etc/letsencrypt` read-only.
