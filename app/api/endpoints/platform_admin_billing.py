"""Platform admin — Billing dashboard metrics."""
...
from app.services.platform_admin_metrics import build_billing_payload
...
def _require_super_admin(admin: PlatformAdmin) -> None:
    ...
    if admin.role != PlatformAdminRole.SUPER_ADMIN.value:
        raise HTTPException(
            ...
            detail="Only Super Admin can view platform billing",
        )
...
async def get_platform_billing(
    ...
    return await build_billing_payload(db)
